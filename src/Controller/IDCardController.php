...
//            $countryCode =  isset($verify_check->data->countryCode) ? $verify_check->data->countryCode : "";
    ...
    public function SendEformData($akcessId, $eid, $label_name, $label) {        
        ...
        if($sql_last_fields['count'] > 0) {
            ...
            $user_type_array = $this->Auth->user( 'usertype' ) ? explode(',',$this->Auth->user( 'usertype' )) : array();
            $data_array = array(
                "request_user" => $akcessId,
                "request_type" => isset($user_type_array[0]) ? $user_type_array[0] : 'Admin',
                "domain_name" => 'edu-kw.akcess.app',
                "akcessId" => $ackess,
