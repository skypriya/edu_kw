...
class EformController extends AppController {
    ...
    //new api 
        ...
        //$eformName = 'pp';
        $saveID = $saveSendId;
        $saveSendId = $this->Global->userIdEncode($saveSendId);
        //$id = 227;
        $fullurl = BASE_ORIGIN_URL.$api.'/'.$saveSendId.'-'.$eformName;
        ...
        if(!empty($response_deeplinking_Data->status) && $response_deeplinking_Data->status == 1) {
            ...
            $doc_id = isset($response_deeplinking_Data->data->doc_id) ? $response_deeplinking_Data->data->doc_id : $saveID;
