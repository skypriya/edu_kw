...
class IDCardController extends AppController {
    ...
    public function recycle() {
        ...
        $this->set('page_title', 'All Recycle IDCard');
    ...
    public function index() {
        ...
        $this->set('page_title', 'All IDCard');
    ...
    public function pdfGenrate() {
        ...
        if($this->request->data('a') != "") {
            ...
            $html .= '.card-box{background-color:#fff;padding:80px 40px;border:2px solid #909090;border-radius:5px;box-sizing:border-box;width:400px;margin:auto;position:relative;text-align: center;}';
            ...
            $html .= '.profile-img-container{padding: 45px 0px;}';
            $html .= '.name{font-size: 28px;text-transform: uppercase;color: #231f20;font-weight: 800;margin: 0;margin-bottom: 10px;}';
            ...
            $html .= 'td{white-space: nowrap;font-size: 20px;line-height: 22px;text-align: left;}';
            ...
            $html .= '<div style="width:170px;height:180px;background-image: url('.$src.');background-size: contain; background-repeat: no-repeat; background-position: center;border-radius: 50%;display:inline-block;border:5px solid #ededed;background-color: #ededed;margin:auto;"></div>';
            $html .= '</div>';
            $html .= '<div class="user-details" style="text-align:center;">';
            $html .= '<p class="name"><b>' . $name . '</b></p>';
            $html .= '<div class="detail mb-20">';
            $html .= '<div class="label-name-light" style="color:gray;font-size:22px;"><b>ID Number : ' . $idNo . '</b></div>';
            $html .= '</div>';
            $html .= '<table>';
            $html .= '<tbody>';
            $html .= '<tr>';
    ...
    public function sendData() {    
        ...
        if($type_doc == 'idcard') {
            ...
            $email_message_type = "Hello, ".COMP_NAME_MESSAGE." sent you the university ID card.";
            $phone_message_type = "Hello, ".COMP_NAME_MESSAGE." sent you the university ID card.";
            $akcess_message_type = "Hello, ".COMP_NAME_MESSAGE." sent you the university ID card.";
            ...
            $email_message_type = "<p>Hi, please check your document '".$doc_name."' for details and download</p>";
            $phone_message_type = "Hi, please check your document '".$doc_name."' for details and download";
            $akcess_message_type = "Hi, ".COMP_NAME_MESSAGE." sent you a document. To check and download.";
        }
        ...
        $query_response = "SELECT email, mobileNumber, akcessId, calling_code FROM `users`
        LEFT JOIN countries ON countries.id = users.country WHERE users.id='" . $fk_users_id . "'";
        $results = $conn->execute($query_response);
                ...
                foreach($emaillist as $key => $value) {
                    ...
                    if($response_deeplinking_Data->status == 1) {
                        ...
                        $data_array = array(
                            ...
                            'html'    => $email_message_type . "<p><a href='" . $deeplink . "'>'".$deeplink."'</a></p>",
                            'recievedType' => $type_doc
                        );
    ...
    public function getDataFROMData(){
        ...
        if(isset($verify_check->data->akcessId) && $verify_check->data->akcessId != "") {
            $akcessId =  isset($verify_check->data->akcessId) ? $verify_check->data->akcessId : "";
            ...
            $dataArray = array(
                ...
                'message' => 'The "' . $akcessId . '" Akcess ID data has been fetched.',
                'status' => 'success'
            );
        } else {
            $dataArray = array(                
