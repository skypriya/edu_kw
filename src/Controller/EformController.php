...
class EformController extends AppController {
    ...
    public function index() {
        ...
        $eform = $this->Eform->find('all' , array('conditions' => ['soft_delete' => 0], 'order'=>array('id' => 'DESC')));    
        ...
        $this->set('page_title', 'EForms');

        $this->set('page_icon', '<i class="fab fa-wpforms mr-1"></i>');
    }
    
    public function SendEformMarketPlaceData($eid) {        
    ...
    public function view($idEncode = null) {
        ...
        foreach ($fields as $n) {
            ...
            $required_class = $field_mandate == 'yes' ? 'required' : '';

            if ( $field_type == 'file') {
                $html .= '<div class="col-12 col-lg-6 ' . $field_ids . '">';
                $html .= '<div class="row">';
                $html .= '<div class="form-group col-12 mb-0">';
                $html .= '<label class="'.$required_class.'">' . $field_label . '</label>';
        ...
        $saveID = $this->Global->userIdEncode($id . "|||||" . $saveSendId);

        $api = 'qrcode/eform';
        //$eformName = 'pp';        
        //$id = 227;
        $fullurl = BASE_ORIGIN_URL.$api.'/'.$this->Global->userIdEncode($eformName).'/'.$saveID;
        // $fullurl = 'https://edu.akcess.app/'.$api.$eformName.'/'.$id;
        ...
        if(!empty($response_deeplinking_Data->status) && $response_deeplinking_Data->status == 1) {

            $deeplink = $response_deeplinking_Data->data->shortLink;
            
            $doc_id = isset($response_deeplinking_Data->data->doc_id) ? $response_deeplinking_Data->data->doc_id : $saveSendId;
            ...
            $dataArray = BASE_ORIGIN_URL . 'api/eform/'.$eformName.'/' . $idEncode;
    ...
    //        $users = $this->Users->find('all', array('conditions' => ['status' => 1,'usertype' => array('Admin','Teacher')]));
    //        $query_data_additional1 = $conn->execute("select * from users where soft_delete != 1 AND status = 1  AND ( usertype = 'Admin' OR usertype = 'Teacher') group by akcessId");
        $query_data_additional1 = $conn->execute("SELECT * FROM users WHERE soft_delete != 1 AND ( FIND_IN_SET('Admin',usertype) > 0 OR FIND_IN_SET('Teacher',usertype) > 0 ) GROUP BY akcessId");
        $users = $query_data_additional1->fetchAll('assoc');

        //for eform process
        $users_process = $this->Users->find('all', array(
            'conditions' => array(
            array(
                'usertype LIKE' => 'Admin%',
                'status'=>1
        ...
        $query = 'SELECT eap.* FROM eform_approval_process as eap
            LEFT JOIN eform as e ON e.id = eap.fk_eform_id
            where eap.soft_delete = 0 AND eap.fk_eform_id = "'.$id.'"';

        $querySql = $conn->execute($query);

        $query_response = $querySql->fetchAll('assoc');

        $eform_approval = array();

        foreach($query_response as $value ){
            $random_number = $value['random_number'];
            $is_notify = $value['is_notify'];
            $is_notify_akcess_id = $value['is_notify_akcess_id'];
            $approval_akcess_id = $value['approval_akcess_id'];
            $eform_approval[$random_number]["approval_akcess_id"] = $approval_akcess_id;
            $eform_approval[$random_number][$is_notify][] = array(
                "is_notify" => $is_notify,
                "is_notify_akcess_id" => $is_notify_akcess_id,
            );
        }

        // echo "<pre>";
        // print_R($eform_approval);
        // exit;
        ...
        $this->set(compact('eform_approval', 'eform', 'id', 'eid', 'fields', 'countries', 'users', 'dataAdditional','users_process'));
    }
    ...
    public function copy($idEncode = null) {
        ...
        if ($id) {
            if($publish == 'yes') {
                $this->SendEformMarketPlaceData($id);
            }
    ...
    public function submitEform() {
        ...
        $isProcess = $this->request->data('process');  
        ...
        if($isProcess == 'yes') {
            ...
            $queryProcessDelete = 'DELETE FROM eform_approval_process where fk_eform_id = "'.$id.'"';

            $conn->execute($queryProcessDelete);
            
            foreach($field as $key => $fields_value_process) {

                $process =  $fields_value_process['process'];
                $notify =  $fields_value_process['notify'];
                
                if(empty($notify)) {
                    $fields_value_process['notify_approve'] = [];

                    $sql_approve_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`) VALUES ('".$key."','".$id."','".$process."','approve')";

                    $conn->execute($sql_approve_notify);
                } else {
                    $notify_approve =  !empty($fields_value_process['notify_approve']) ? $fields_value_process['notify_approve'] : [];
                
                    if($notify_approve) {

                        foreach($notify_approve as $notify_approve_value) {

                            $sql_approve_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`, `is_notify_akcess_id`) VALUES ('".$key."','".$id."','".$process."','approve','".$notify_approve_value."')";
                ...
                if(empty($notify)) {
                    $fields_value_process['notify_reject'] = [];

                    $sql_approve_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`) VALUES ('".$key."','".$id."','".$process."','reject')";

                    $conn->execute($sql_approve_notify);
                } else {

                    $notify_reject =  !empty($fields_value_process['notify_reject']) ? $fields_value_process['notify_reject'] : [];

                    if($notify_reject) {

                        foreach($notify_reject as $notify_reject_value) {

                            $sql_reject_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`, `is_notify_akcess_id`) VALUES ('".$key."','".$id."','".$process."','reject','".$notify_reject_value."')";

                            $conn->execute($sql_reject_notify);
                ...
                if(empty($notify)) {
                    $fields_value_process['notify_both'] = [];

                    $sql_approve_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`) VALUES ('".$key."','".$id."','".$process."','both')";

                    $conn->execute($sql_approve_notify);
                } else {

                    $notify_both =  !empty($fields_value_process['notify_both']) ? $fields_value_process['notify_both'] : [];

                    if($notify_both) {

                        foreach($notify_both as $notify_both_value) {

                            $sql_both_notify = "INSERT INTO `eform_approval_process` (`random_number`,`fk_eform_id`, `approval_akcess_id`, `is_notify`, `is_notify_akcess_id`) VALUES ('".$key."','".$id."','".$process."','both','".$notify_both_value."')";

                            $conn->execute($sql_both_notify);
        ...
        if($isAdditionalNotification == 'yes') {
            ...
            $additionalNotificationTo = 1;
            
            $akcessID = $this->request->data('akcessID');  
            
            $queryNotificationDelete = 'DELETE FROM additionalnotificationto where fk_eform_id = "'.$id.'"';

            $conn->execute($queryNotificationDelete);
        ...
        } else if($isAdditionalNotification == 'no'){
        ...
        if (!empty($send_invitation_eform) && $send_invitation_eform != "") {
            $send_invitation_eform_update = [
                'send_invitation_eform'     => ""
            ];

            $this->Eform->updateAll(
                $send_invitation_eform_update,
                [
                    'send_invitation_eform' => ucfirst($send_invitation_eform)
                ]
            );
        }
        ...
        $isProcesspdf = $this->request->data('processpdf');  

        $eform_response_process_pdf = json_encode($this->request->data('eform_response_process_pdf'));  

        if($isProcesspdf == 'no'){
            $eform_response_process_pdf = null;  
        }
        ...
        $eform_after = [
            ...
            'isclientInvitationEform'   => $isclientInvitationEform,
            'send_invitation_eform'     => $send_invitation_eform,
            'isProcesspdf'              => $isProcesspdf,
            'eform_response_process_pdf'=> $eform_response_process_pdf,
        ];
        ...
        $eform_before = [
            ...
            'is_approval'               => $eformDetails_before->is_approval,
            ...
            'isProcesspdf'              => $eformDetails_before->isProcesspdf,
            'eform_response_process_pdf'=> $eformDetails_before->eform_response_process_pdf,
        ];        
        ...
        if($is_approval == 1) {
            ...
            if($result_approval_sql) {
                ...
                foreach($result_approval_sql as $result_sqls) {
                    ...
                    $is_notify_akcess_id = isset($result_sqls['is_notify_akcess_id']) && $result_sqls['is_notify_akcess_id'] ? $result_sqls['is_notify_akcess_id'] : 0;
                ...
                $is_approval_json_child = '';

                foreach($approved_array as $approved_arrays) {

                    $approved_akcessId = $approved_arrays['approved_akcessId'];
                    $isApprovedNotification = $approved_arrays['isApprovedNotification'];
                    $is_approval_json_child .= '{';
                    $is_approval_json_child .= '"approved_akcessId" : "'.$approved_akcessId.'",';
                    $is_approval_json_child .= '"isApprovedNotification" : "'.$isApprovedNotification.'",';
                    $is_approval_json_child .= '"approvedNotification" : [';

                    $is_approval_json_sub_child = '';

                    foreach($approved_arrays['approved'] as $approved) {

                        $approved_akcessId = $approved['approved_akcessId'];
                        $type_approval = $approved['type'];

                        $is_approval_json_sub_child .= '{';
                        $is_approval_json_sub_child .= '"approved_akcessId" : "'.$approved_akcessId.'",';
                        $is_approval_json_sub_child .= '"type" : "'.$type_approval.'"';
                        $is_approval_json_sub_child .= '},';

                    }

                    $is_approval_json_child .= rtrim($is_approval_json_sub_child, ',');
                ...
                $is_approval_json .= rtrim($is_approval_json_child, ',');
        ...
        if(isset($type) && $inlineRadioOptions == 'phone') {
            ...
            if($field_phone) {
                ...
                foreach($field_phone as $key => $value) {
                    ...
                    $api_url_merge = 'qrcode/eform';

                    $saveID = $this->Global->userIdEncode($id . "|||||" . $saveSendId);
        
                    $fullurl = BASE_ORIGIN_URL.$api_url_merge.'/'.$this->Global->userIdEncode($formName).'/'.$saveID;
                ...
                foreach($emaillist as $key => $value) {
                    ...
                    if($response_deeplinking_Data->status == 1) {
                        ...
                        $data_array = array(
                            ...
                            'html'    => "<p>Hi ".$email.", </p><p>eForm Name : ".$formName."</p><p>".strtoupper(COMP_NAME)." sent you an eForm. Please click on the below url to open it in AKcess app
                            <p><a href='" . $deeplink . "'>Click Here</a>",
                            'recievedType' => $type_doc
                        );
...
//                            $data['data'] = "The " . $description_title . " has been sent successfully.";
                ...
                foreach($ackess as $akcessIds) {
                    ...
                    //if(isset($verify_check->data->akcessId) && $verify_check->data->akcessId != "") {
                        ...
                        if($response_akcess_Data->status == 1) {
                            ...
                            $data['message'] = "error";
                            $data['data'] = isset($response_akcess_Data->message) ? $response_akcess_Data->message : "type is not set!";
                        }
                    //} else {
                    //    $data['message'] = "error";
    ...
    public function delete($idEncode = null) {
        ...
        if ($updateEform) {
            ...
            $this->deleteEformMarketPlaceData($eform_main_id);
    ...
    public function deleteEformMarketPlaceData($eform_main_id) {        
           
        $api = isset($_POST['api']) ? $_POST['api'] : SITE_API_KEY_URL;
        $origin_url = isset($_POST['orurl']) ? $_POST['orurl'] : ORIGIN_URL;
        $api_url = isset($_POST['apiurl']) ? $_POST['apiurl'] : AK_ORIGIN_URL;    
                    
        $eformid = $eform_main_id;
        ...
        //delete-marketplace-eforms 
        //https://ak-api-v2.akcess.dev/eforms/5fdb24e632731b31c14b94ec/market-place
        //https://ak-api-v2.akcess.dev/eforms/kvos0hqat7p5u4ydcixw1mg2/market-place


        $type_method = 'eforms/' . $eformid . '/market-place';

        $method = "DELETE";   

        $data_array = array();

        $response_data_delete = $this->Global->curlGetPostEform($method, $type_method, $api, $origin_url, $api_url, $data_array, $origin_array);    
        
        $response_delete_Data = json_decode($response_data_delete);  
