                    ...
                    <div class="table-responsive m-t-10">
                        ...
                        <?php if (in_array('Admin',$session_usertype)) { ?>
                            ...
                            <div class="row float-right m-0">
                                <h4><button type="button" class="btn waves-effect waves-light btn-info mr-3" onclick="sendInvitationModalModule('teacher');" data-title="teacher">Send Invitation</button></h4>
                                <h4><?php echo $this->Html->link(__('<i class="fas fa-plus mr-1"></i> Add Academic Personnel'), ['action' => 'addTeacher'], ['class' => 'btn waves-effect waves-light btn-primary mr-3', 'escape' => false]); ?></h4>
                                <h4><?php echo $this->Html->link(__('<i class="fa fa-recycle"></i>'), ['action' => 'teacherRecycle'], ['class' => 'btn waves-effect waves-light btn-warning', 'escape' => false, 'data-toggle' => "tooltip", 'title'=>'Recycle']) ?></h4>
                            </div>
                        <?php } ?>
                                 ...
                                 <?php $k = 0; foreach ($users as $user): ?>
                                    <?php $user_id = ConnectionManager::userIdEncode($user['id']); ?>
                                     <tr>
                                        ...
                                        <td>
                                            <?= $this->Html->link(__($user['name']), ['action' => 'view-teacher', $user_id], ['escape' => false, 'data-toggle' => "tooltip", 'title'=>'View']) ?> 
                                        </td>                                        
                                        <td><?= h($user['email']) ?></td>
                                        <td><?= h($user['mobileNumber']) ?></td>
                                        <td class="actions">
                                            <div class="actions-div">
                                                <?= $this->Html->link(__('<i class="fa fa-eye"></i>'), ['action' => 'view-teacher', $user_id], ['escape' => false, 'data-toggle' => "tooltip", 'title'=>'View']) ?>

                                                <?php if (in_array('Admin',$session_usertype)) { ?>
                                                
                                                <?= $this->Html->link(__('<i class="fa fa-edit green-txt"></i>'), ['action' => 'edit-teacher', $user_id], ['escape' => false, 'data-toggle' => "tooltip", 'title'=>'Edit']) ?>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-sm">
        <div class="modal-content">
            <div class="modal-body">
                <p class="text-center">This user will be removed from all classes assigned to him if he is a teacher. Are you sure you want to delete?</p>
            </div>
        ...
        <div class="modal-content">
            <form role="form" action="<?php echo $url; ?>messaging/SendInvitationData" id="SendInvitationData" method="POST" class="mb-0">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-info waves-effect" id="submit" onclick="sendInvitationData()">Send</button>
                </div>
            </form> 
        </div>
    </div>
...
function list_add_button() {
    ...
    html += '<a href="javascript:void(0);" class="btn waves-effect waves-light btn-danger" onclick="list_remove_button('+x+')"><i class="fa fa-minus"></i></a>';
    html += '</div>';
    html += '</div>';
...
function searchUserType(){
    ...
    document.location.href = burl + '/users/teacher-list?s=' + strUserType;
}
